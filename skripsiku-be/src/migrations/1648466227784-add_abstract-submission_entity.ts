...
export class addAbstractSubmissionEntity1648466227784 implements MigrationInterface {
    name = 'addAbstractSubmissionEntity1648466227784'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`abstract_submissions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`title\` varchar(255) NOT NULL, \`status\` int NOT NULL, \`sequence\` int NOT NULL, \`user_id\` int NOT NULL, \`attachment\` int NULL, \`submission_period_id\` int NOT NULL, UNIQUE INDEX \`REL_e8a0284e69854693ab8f224547\` (\`attachment\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`REL_e8a0284e69854693ab8f224547\` ON \`abstract_submissions\``);
        await queryRunner.query(`DROP TABLE \`abstract_submissions\``);
    }
