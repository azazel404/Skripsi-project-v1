...
export class addedSubmissionAttachmentApproverEntity1650118231125 implements MigrationInterface {
    name = 'addedSubmissionAttachmentApproverEntity1650118231125'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`submission_attachment_approvers\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`level\` int NULL, \`lecturer_id\` int NOT NULL, \`user_id\` int NOT NULL, \`submission_attachment_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP TABLE \`submission_attachment_approvers\``);
    }
