...
export class addSubmissionPeriodEntity1647679470130 implements MigrationInterface {
    name = 'addSubmissionPeriodEntity1647679470130'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP INDEX \`IDX_93a5586a540352e3c3719840d7\` ON \`lecturers\``);
        await queryRunner.query(`CREATE TABLE \`submission_periods\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`start_date\` datetime NOT NULL, \`end_date\` datetime NOT NULL, \`major\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP TABLE \`submission_periods\``);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_93a5586a540352e3c3719840d7\` ON \`lecturers\` (\`profile_picture\`)`);
    }
