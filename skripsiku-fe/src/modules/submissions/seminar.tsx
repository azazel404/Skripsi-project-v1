...
const SeminarAssessment = (props: any) => {
    ...
    const [isOpenCreateSeminar, setIsOpenCreateSeminar] = useState(false);
    ...
    useEffect(() => {
        ...
        if (detailSubmission && detailSubmission.latest_stage >= 2) {
    ...
    const onUpdateAndCreateSeminar = (values: any) => {
        setIsLoading(true);
        ...
        let fetch: any;
        let msg: any;
        if (isOpenScheduleSeminar) {
            fetch = SubmissionAPI.updateSeminarByLecturer(payload, data.id);
            msg = "Berhasil Ubah Jadwal Seminar";
        }
        else if (isOpenCreateSeminar) {
            payload.submission_id = detailSubmission.id
            fetch = SubmissionAPI.createSeminar(payload)
            msg = "Berhasil Buat Jadwal Seminar";
        }
        fetch
            .then(res => {
                dispatch(openNotificationToast(msg));
                if (isOpenScheduleSeminar) {
                    setIsOpenScheduleSeminar(false);
                }
                if (isOpenCreateSeminar) {
                    setIsOpenCreateSeminar(false);
                }
                refreshSubmission()
            })
            .catch((err: any) => {
    ...
    const takeActionProposal = (values: any) => {
        setIsLoading(true)
        let payload;
        if (parseInt(values.status) === 2 || parseInt(values.status) === 3) {
            payload = {
                ...values,
                score_content: 0,
    ...
    //                 <Typography style={{ fontSize: '12px', fontWeight: 'bold', color: '#D32F2F' }}>Direvisi.</Typography>
    //             </div>
    //         )
    //     }
    //     else if (status === 3) {
    ...
    //             <div style={{ display: 'flex' }}>
    //                 <Typography style={{ fontSize: '12px', marginRight: '4px' }}>Status diubah menjadi</Typography>
    //                 <Typography style={{ fontSize: '12px', fontWeight: 'bold', color: '#D32F2F' }}>Ditolak.</Typography>
    //             </div>
    //         )
    //     }
    // }
                    ...
                    return (
                        <div>
                            <Button
                                ...
                                size="small"
                                sx={{ mr: 2 }}
                                onClick={() => setIsOpenCreateSeminar(true)}
                            >
                                Buat Jadwal Seminar
                            </Button>
                        </div>
                    ...
                    return (
                        <div>
                            <Button
                                ...
                                variant="contained"
                                size="small"
                                onClick={() => setIsOpenScheduleFinal(true)}
                            >
                                Buat Jadwal Ujian Sidang
                            </Button>
                        </div>
                    )
                }
    ...
    const renderButtonUpdateSchedule = () => {
        ...
        if (profileId === thesis_id) {
            //NOTE perlu pengecekan reject status
            if (data.status !== 2) {
                return (
            ...
            render = (
                <>
                    ...
                    <div>
                        ...
                        {/* {data.status !== 0 ? "" : showButtonEvaluation()} */}
                    </div>
                </>
            )
    ...
    return (
        <>
            ...
            <Grid container spacing={2} style={{ marginTop: '8px' }}>
                ...
                <ScheduleSeminarModal isLoading={isLoading} type="seminar" open={isOpenCreateSeminar} handleClose={() => setIsOpenCreateSeminar(false)} onSubmitModal={(payload: any) => onUpdateAndCreateSeminar(payload)} />
                <UpdateScheduleModal isLoading={isLoading} open={isOpenScheduleSeminar} handleClose={() => setIsOpenScheduleSeminar(false)} onSubmitModal={(payload: any) => onUpdateAndCreateSeminar(payload)} />
