...
const Majors = (props: any) => {
    ...
    const saveData = (values: any) => {
        ...
        let fetch;
        if (values.id) {
            fetch = MajorAPI.update(values.id, payload)
        }
        else {
            fetch = MajorAPI.create(payload)
        }
        fetch
            .then((res: any) => {
                toggleOpenModal();
        ...
        <>
            ...
            >
                <Container maxWidth={false}>
                    <ToolbarComponent title="Program Studi" customRender={customToolbarActions} />
                    <Box sx={{ mt: 3 }}>
                        <TableDynamic
...
Majors.getLayout = (page: any) => <DashboardLayout>{page}</DashboardLayout>;
